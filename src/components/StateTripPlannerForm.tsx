...
import { stateTripPlannerFormSchema } from '@/utils/validations/itirenaryValidations';
import {
  FormLabel,
  Form,
  FormField,
...
export default function StateTripPlannerFormSchema() {
  ...
  const form = useForm<z.infer<typeof stateTripPlannerFormSchema>>({
    resolver: zodResolver(stateTripPlannerFormSchema),
    defaultValues: {
      tripDays: 1,
      placesPerDay: 1,
      country: '',
  ...
  async function onSubmit(data: z.infer<typeof stateTripPlannerFormSchema>) {
    ...
    try {
      startTransition(async () => {
        if (data.country === '' || data.state === '') {
          toast({
            title: 'Error',
            description: 'Please select a country, state',
            duration: 5000,
            variant: 'destructive',
          });
          return;
